...
Output: {
  "intent": "general_chat",
  "action": { "type": "respond" },
  "response": "Hi there! 👋 I'm here to help you with anything related to MyRush. Looking for a court, or need info on a venue?"
}
`;
};
...
export const getGeminiResponse = async (userMessage: string, conversationHistory: any[] = []) => {
  try {
    if (!API_KEY) {
      throw new Error("Gemini API Key is missing. Check VITE_GEMINI_API_KEY in .env");
    }
